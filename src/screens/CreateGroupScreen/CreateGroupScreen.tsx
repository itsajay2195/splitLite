...
type MemberEntry = { name: string; upiId: string };
...
export default function CreateGroupScreen() {
  ...
  const createGroup = useCallback(async () => {
    ...
    try {
      ...
      realm.write(() => {
        realm.create('Group', { _id: groupId, name: trimmedName, createdAt: new Date() });
      ...
      // Sync to Firestore if logged in (non-blocking)
      syncGroupCreated(realm, groupId);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.form}>
        ...
        <TextInput placeholder="Name" placeholderTextColor={colors.text3} style={styles.input} value={memberInput} onChangeText={setMemberInput} returnKeyType="next" />
        <View style={styles.upiRow}>
          <TextInput placeholder="UPI ID (optional, e.g. name@upi)" placeholderTextColor={colors.text3} style={[styles.input, { flex: 1 }]} value={upiInput} onChangeText={setUpiInput} onSubmitEditing={addMember} returnKeyType="done" autoCapitalize="none" keyboardType="email-address" />
          ...
          renderItem={({ item, index }) => (
            <View style={styles.memberChip}>
              <View style={{ flex: 1 }}>
                ...
                {item.upiId ? <Text style={{ color: colors.text3, fontSize: 11, marginTop: 2 }}>{item.upiId}</Text> : null}
              </View>
...
const styles = StyleSheet.create({
  ...
  memberChip: { backgroundColor: colors.surface, padding: 12, paddingHorizontal: 14, borderRadius: 14, marginBottom: 8, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: colors.border },
  saveBtn: { backgroundColor: colors.accent, padding: 16, borderRadius: 16, marginTop: 'auto' },
