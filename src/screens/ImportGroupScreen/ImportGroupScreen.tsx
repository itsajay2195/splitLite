...
import { syncGroupCreated } from '../../services/firestoreSync';
...
export default function ImportGroupScreen() {
  ...
  const applyPayload = (payload: any, existingId: Realm.BSON.ObjectId, isSyncing: boolean) => {
    realm.write(() => {
      if (!isSyncing) {
        realm.create('Group', { _id: existingId, name: payload.group.name, createdAt: new Date(payload.group.createdAt) });
      }

      payload.members?.forEach((m: any) => {
        const mId = new Realm.BSON.ObjectId(m.id);
        if (!realm.objectForPrimaryKey('Member', mId)) {
          realm.create('Member', { _id: mId, groupId: existingId, name: m.name, upiId: m.upiId ?? undefined });
        }
      });
      ...
      payload.expenses?.forEach((e: any) => {
        ...
        if (!realm.objectForPrimaryKey('Expense', eId)) {
          realm.create('Expense', { _id: eId, groupId: existingId, amount: e.amount, paidByMemberId: new Realm.BSON.ObjectId(e.paidBy), description: e.desc ?? '', date: new Date(e.date), category: e.category ?? undefined });
          payload.splits?.filter((s: any) => s.expenseId === e.id).forEach((s: any) => {
            realm.create('ExpenseSplit', { _id: new Realm.BSON.ObjectId(), expenseId: eId, memberId: new Realm.BSON.ObjectId(s.memberId), amount: s.amount });
          });
        }
      });
      ...
      payload.payments?.forEach((p: any) => {
        ...
        if (!realm.objectForPrimaryKey('Payment', pId)) {
          realm.create('Payment', { _id: pId, groupId: existingId, fromMemberId: new Realm.BSON.ObjectId(p.from), toMemberId: new Realm.BSON.ObjectId(p.to), amount: p.amount, date: new Date(p.date) });
        }
      });
    });

    // Sync imported group to Firestore if logged in
    if (!isSyncing) syncGroupCreated(realm, existingId);
  };

  const importGroup = (json: string) => {
    try {
      const payload = JSON.parse(json);
      if (typeof payload.v === 'undefined' || !payload.group || !Array.isArray(payload.members)) {
      ...
      if (existing) {
        showAlert({
          title: 'Sync group?',
          message: `"${payload.group.name}" is already in your groups. Sync to get the latest?`,
          buttons: [
            { text: 'Cancel', style: 'cancel', onPress: () => setScanned(false) },
            {
              text: 'Sync', style: 'default',
              onPress: () => {
                applyPayload(payload, existingId, true);
                showAlert({
                  title: 'Synced!',
                  message: `"${payload.group.name}" is up to date.`,
                  buttons: [{ text: 'View Group', style: 'default', onPress: () => navigation.replace('Group', { groupId: payload.group.id }) }],
                });
              },
            },
          ],
  ...
  if (hasPermission === null) return <View style={styles.center}><ActivityIndicator color={colors.accent} /></View>;
  if (!hasPermission) {
    return (
  ...
  if (!device) return <View style={styles.center}><Text style={styles.permText}>No camera found on this device.</Text></View>;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.overlay}>
        ...
        <Text style={styles.hint}>{scanned ? 'Processing...' : 'Point at a Baagam QR code'}</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  backBtn: { backgroundColor: colors.surface2, paddingHorizontal: 24, paddingVertical: 12, borderRadius: 12, borderWidth: 1, borderColor: colors.border },
  backBtnText: { color: colors.text, fontWeight: '600' },
  overlay: { ...StyleSheet.absoluteFillObject, justifyContent: 'center', alignItems: 'center' },
  closeBtn: { position: 'absolute', top: 56, right: 20, backgroundColor: 'rgba(0,0,0,0.5)', width: 40, height: 40, borderRadius: 20, justifyContent: 'center', alignItems: 'center' },
  ...
  hint: { color: 'rgba(255,255,255,0.7)', marginTop: 32, fontSize: 14 },
});
